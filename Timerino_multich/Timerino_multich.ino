...
/*
 * 
 * DRF - Added rounding to f/stop stepping for repeatable up/down steps
 * DRF - Fixed missing conditional compilation statements for LCD operation and LED dimming
 * DRF - Change relay output to active low to match relay module
 * DRF - Eliminated startp scroll brag line for faster startup/reset
 * DRF - Fixed LCD clearing issues, extraneous letters left on screen. other formatting issues
 * DRF - Initialized button and mode pins with internal pullups, active low for cleaner operation
 * DRF - Changed button sense to active low to match new button configuration
 * DRF - Changed mode switch to pushbutton with toggle
 * DRF - Fixed bug in init_timermode() function, case MODFSTDOWN: - "appo_time = time_fstdown[ch];" not "appo_time = ncifra_fstdown[ch];"
 * DRF - Add current channel display to Setup Mode selection
 * 
...
void say_free() {
  ...
  say_clearprecis();
  say_cleartime();
  say_ch();  //DRF - Display current channel
...
void test_strip() {
  if (millis() - last_time >= 100) {
    ...
    if (time == time_succ) {
      ...
      time_succ = int(round(float(time) * mult)); //DRF - Do proper round, not truncate
    }
    if (time_succ - time <= 3) {
      beep(tone_up, 50);
    }
    ...
    switch (key) {
      ...
      case '*': 
        if (timer_mode == MODFSTDOWN) {
          // Step down
          mult = pow(2.0, (1.0/precis));
          time = int(round(float(time) / mult)); //DRF - Do proper round, not truncate
          ...
          scherma = true;
          mult = 1.0/precis;
          time_burn = int(round(float(time) * mult)); //DRF - Do proper round, not truncate
